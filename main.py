...
frame_detalhes.grid(row=4, column=0, pady=10, sticky=NSEW)

frame_tabela = Frame(janela, width=850, height=200, bg=co1)
frame_tabela.grid(row=5, column=0, pady=10, padx=10, sticky=NSEW)

# Trabalhando no frame Logo ---------------------------------------------
app_lg = Image.open('logo.png')
app_lg = app_lg.resize((50, 50))
app_lg = ImageTk.PhotoImage(app_lg)
app_logo = Label(frame_logo, image=app_lg, text="Cadastro de Alunos", width=850, compound=LEFT, relief=RAISED, anchor=NW, font=('Ivy 15 bold'), bg=co10, fg=co1)
...
def alunos():
    ...
    def update_aluno():
        ...
        try:
            ...
            def update():
                ...
                # Destruindo o botão apos salvar
                botao_update.destroy()

            botao_update = Button(frame_detalhes, text="Atualizar".upper(), width=9, command=update,
                                  compound=CENTER, anchor=CENTER, overrelief=RIDGE, font=('Ivy 7'),
                                  bg=co3, fg=co1)
    ...
    def delete_aluno():
        try:
            ...
            valor_id = tree_lista[0]
            view.deletar_aluno([valor_id])

            # Monstrando a mensagem de sucesso
    ...
    c_sexo['values'] = ['Masculino','Feminino', 'Não Binário']
    ...
    botao_carregar = Button(frame_detalhes, command=escolher_imagem,
                    text="Carrehar Foto".upper(), width=20,
                    compound=CENTER, anchor=CENTER, overrelief=RIDGE, font=('Ivy 7'),
                    bg=co1, fg=co0)
    ...
    botao_salvar = Button(frame_detalhes, text="Salvar".upper(), width=9, command=novo_aluno,
                            compound=CENTER, anchor=CENTER, overrelief=RIDGE, font=('Ivy 7'),
                            bg=co3, fg=co1)
    botao_salvar.place(x=627, y=110)

    botao_atualizar = Button(frame_detalhes, command=update_aluno, text="atualizar".upper(), width=9,
                            compound=CENTER, anchor=CENTER, overrelief=RIDGE, font=('Ivy 7'),
                            bg=co6, fg=co1)
    botao_atualizar.place(x=627, y=135)

    botao_deletar = Button(frame_detalhes, command=delete_aluno, text="Deletar".upper(), width=9,
                            compound=CENTER, anchor=CENTER, overrelief=RIDGE, font=('Ivy 7'),
                            bg=co7, fg=co1)
    botao_deletar.place(x=627, y=160)

    botao_ver = Button(frame_detalhes, text="Ver".upper(), width=9,
                            compound=CENTER, anchor=CENTER, overrelief=RIDGE, font=('Ivy 7'),
                            bg=co1, fg=co0)
    ...
    # Tabela Alunos
    def mostrar_alunos():
        app_nome = Label(frame_tabela, text='Tabela de estudantes', height=1, pady=0, padx=0,
                         relief='flat', anchor=NW, font='Ivy 10 bold', bg=co1, fg=co4)
        app_nome.grid(row=0, column=0, padx=0, sticky=NSEW)

        # criando uma visualização com barras de rolagem duplas
        list_header = ['ID', 'Nome', 'email', 'Telefone', 'sexo', 'imagem', 'Data', 'CPF', 'Turma']
        df_list = view.ver_alunos()
...
# função para adicionar Cursos e Turmar
def adicionar():
    ...
    def novo_curso():
        ...
        # Verificando se os valores estão fazios ou nao
        for i in lista:
            if i == "":
    ...
    botao_carregar = Button(frame_detalhes, command=novo_curso, anchor=CENTER, text='Novo Curso'.upper(), width=10, overrelief=RIDGE, font='Ivy 7 bold', bg=co3, fg=co1)
    ...
    def mostrar_cursos():
        app_nome = Label(frame_tabela_curso, text='Tabela de Cursos', height=1, pady=0, padx=0,
                         relief='flat', anchor=NW, font='Ivy 10 bold', bg=co1, fg=co4)
        app_nome.grid(row=0, column=0, padx=0, sticky=NSEW)

        #criando uma visualização com barras de rolagem duplas
        list_header = ['ID', 'Curso', 'Duração', 'Preço']
        df_list = view.ver_cursos()
    ...
    # Tabela Turma
    def mostrar_turmas():
        app_nome = Label(frame_tabela_turma, text='Tabela de Turmas', height=1, pady=0, padx=0,
                         relief='flat', anchor=NW, font='Ivy 10 bold', bg=co1, fg=co4)
        app_nome.grid(row=0, column=0, padx=0, sticky=NSEW)

        #criando uma visualização com barras de rolagem duplas
        list_header = ['ID', 'Nome da Turma', 'Curso', 'Inicio']
        df_list = view.ver_turmas()
...
app_cadastro = Button(frame_dados, command=lambda:controle('cadastro'),
...
app_adicionar = Button(frame_dados, command=lambda:controle('adicionar'),
